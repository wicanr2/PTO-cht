#!/usr/bin/env python3
"""Scratch debug v2: cave at seg110:0x00 —
1) if ds:0x8098==0: call grpseg loader seg11:0x2154
2) if stacked ptr arg seg==0: rewrite stacked arg from ds:[0x8096]/[0x8098]
3) dump helper stack (16 bytes) to stderr
4) run original helper prologue, jmp back to seg5:0x5b78
"""
import struct, sys
...
cave = bytes([
    ...
    0x9A, 0xFF, 0xFF, 0x00, 0x00,              # 09 lcall seg11:0x2154 (site 0x0a)
    0x8B, 0x44, 0x20,                          # 0e mov ax,[sp+0x1c] (stacked ptr seg)
    0x85, 0xC0,                                # 11 test ax,ax
    0x75, 0x0D,                                # 13 jne 0x22
    0xA1, 0x96, 0x80,                          # 15 mov ax,[0x8096]
    0x89, 0x44, 0x1E,                          # 18 mov [sp+0x1a],ax
    0x8B, 0x16, 0x98, 0x80,                    # 1b mov dx,[0x8098]
    0x89, 0x54, 0x20,                          # 1f mov [sp+0x1c],dx
    0x16,                                      # 22 push ss
    0x1F,                                      # 23 pop ds
    0x89, 0xE2,                                # 24 mov dx,sp
    0x83, 0xC2, 0x14,                          # 26 add dx,0x14
    0xB9, 0x10, 0x00,                          # 29 mov cx,0x10
    0xBB, 0x02, 0x00,                          # 2c mov bx,2
    0xB4, 0x40,                                # 2f mov ah,0x40
    0xCD, 0x21,                                # 31 int 0x21
    0x1F,                                      # 33 pop ds
    0x61,                                      # 34 popa
    0xC8, 0x06, 0x00, 0x00,                    # 35 enter 6,0
    0x57,                                      # 39 push di
    0x56,                                      # 3a push si
    0xEA, 0xFF, 0xFF, 0x00, 0x00,              # 3b jmp far seg5:0x5b78 (site 0x3c)
])
assert len(cave) == 0x40
...
add_reloc(110, 0x003C, 5, 0x5B78)
open(DST, 'wb').write(data)
print(f'{DST}: v2 loader+fix+dump patch applied')
